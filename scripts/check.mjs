...
const packedAllowlist = [
  ...
  'package/lib/types/client/AutomationsApp.d.ts',
  'package/lib/types/client/vendor/protocol.d.ts',
  'package/lib/types/client/vendor/runtime.d.ts',
].sort()

function fail(message) {
  throw new Error(`dsh-automations-app check failed: ${message}`)
}
...
function assertPackedPayload() {
  const directory = mkdtempSync(join(tmpdir(), 'dsh-automations-app-pack-'))
  try {
...
if (mode === '--lint') {
  await assertSources()
  console.log('Automations App lint/source checks passed')
} else if (mode === '--pack') {
  await assertSources()
  await assertBuilt()
  assertPackedPayload()
  console.log('Automations App packed payload checks passed')
} else {
